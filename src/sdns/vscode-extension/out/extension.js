...
let keywordSources = new Map();
let allKeywords = new Map();
 ...
 */
function loadKeywordFiles(workspaceRoot) {
    ...
    for (const file of files) {
        try {
            ...
            const keywordFile = JSON.parse(content);
 ...
 */
function getKeywordsForCategory(source, category) {
    const file = keywordSources.get(source);
    if (!file || !file.keywords[category])
        return [];
    const keywords = [];
...
function activate(context) {
    ...
    if (workspaceFolders && workspaceFolders.length > 0) {
        ...
        const keywordWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(workspaceFolders[0], 'src/data/keywords/definitions/*.keywords.json'));
        ...
        const keysWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(workspaceFolders[0], 'src/patients/**/*.keys'));
    ...
    const keywordInfoCommand = vscode.commands.registerCommand('sdns.showKeywordInfo', (keywordId) => {
        const keyword = getKeywordById(keywordId);
        if (keyword) {
            const panel = vscode.window.createWebviewPanel('sdnsKeywordInfo', `Keyword: ${keyword.displayText}`, vscode.ViewColumn.Beside, {});
    ...
    context.subscriptions.push(completionProvider, hoverProvider, symbolProvider, definitionProvider, foldingProvider, keywordInfoCommand, reloadKeywordsCommand, validateOnChange, validateOnOpen);
 ...
 */
function validateKeysFile(uri, diagnostics) {
    try {
 ...
 */
function validateSdnsDocument(document, problems) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmed.startsWith('@if ') || trimmed === '@if') {
            ...
            ifDepth--;
            if (ifDepth < 0) {
                problems.push(new vscode.Diagnostic(new vscode.Range(i, 0, i, trimmed.length), 'Unexpected @endif without matching @if', vscode.DiagnosticSeverity.Error));
                ifDepth = 0;
            }
        }
        ...
        if (trimmed.startsWith('@when ') || trimmed.startsWith('@when(')) {
            ...
            whenDepth--;
            if (whenDepth < 0) {
                problems.push(new vscode.Diagnostic(new vscode.Range(i, 0, i, trimmed.length), 'Unexpected @end without matching @when', vscode.DiagnosticSeverity.Error));
                whenDepth = 0;
            }
        }
        // Track multi-line string state
        ...
        for (const match of keywordRefs) {
            ...
            if (!getKeywordById(keywordId)) {
                ...
                problems.push(new vscode.Diagnostic(new vscode.Range(i, startCol, i, startCol + match[0].length), `Unknown keyword: ${keywordId}`, vscode.DiagnosticSeverity.Warning));
            }
        }
        ...
        for (const match of animRefs) {
            ...
            if (!validAnims.includes(animId)) {
                const startCol = line.indexOf(match[0]);
                problems.push(new vscode.Diagnostic(new vscode.Range(i, startCol, i, startCol + match[0].length), `Unknown animation: ${animId}. Valid animations: ${validAnims.join(', ')}`, vscode.DiagnosticSeverity.Warning));
            }
        }
        ...
        for (const match of styleRefs) {
            ...
            if (styleParts.length !== 2) {
                const startCol = line.indexOf(match[0]);
                problems.push(new vscode.Diagnostic(new vscode.Range(i, startCol, i, startCol + match[0].length), `Style reference should be in format 'category.preset' (e.g., behavior.red)`, vscode.DiagnosticSeverity.Warning));
            }
            else if (!validStyleCategories.includes(styleParts[0])) {
                const startCol = line.indexOf(match[0]);
                problems.push(new vscode.Diagnostic(new vscode.Range(i, startCol, i, startCol + match[0].length), `Unknown style category: ${styleParts[0]}. Valid: ${validStyleCategories.join(', ')}`, vscode.DiagnosticSeverity.Warning));
            }
        }
        ...
        if (gotoMatch) {
            ...
            if (!targetPattern.test(text) && !target.includes('/')) {
                problems.push(new vscode.Diagnostic(new vscode.Range(i, trimmed.indexOf(target), i, trimmed.indexOf(target) + target.length), `Goto target not found: ${target}`, vscode.DiagnosticSeverity.Information));
            }
        }
    }
    // Report unclosed @if blocks
    if (ifDepth > 0) {
        problems.push(new vscode.Diagnostic(new vscode.Range(0, 0, 0, 1), `${ifDepth} unclosed @if block(s) - missing @endif`, vscode.DiagnosticSeverity.Error));
    }
    // Report unclosed @when blocks
    if (whenDepth > 0) {
        problems.push(new vscode.Diagnostic(new vscode.Range(0, 0, 0, 1), `${whenDepth} unclosed @when block(s) - missing @end`, vscode.DiagnosticSeverity.Error));
    }
    // Report unclosed multi-line strings
    if (inMultiLineString) {
        problems.push(new vscode.Diagnostic(new vscode.Range(multiLineStringStart, 0, multiLineStringStart, lines[multiLineStringStart].length), 'Unclosed string literal - missing closing quote', vscode.DiagnosticSeverity.Error));
    }
}
/**
 * Validate a .keys document
 */
function validateKeysDocument(document, problems) {
    const text = document.getText();
    const lines = text.split('\n');
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (braceDepth < 0) {
            problems.push(new vscode.Diagnostic(new vscode.Range(i, 0, i, line.length), 'Unexpected closing brace', vscode.DiagnosticSeverity.Error));
            braceDepth = 0;
        }
        ...
        if (currentSection === 'KEYWORDS' && braceDepth === 0) {
            ...
            if (keywordDefMatch) {
                ...
                if (!keywordId.includes('.')) {
                    problems.push(new vscode.Diagnostic(new vscode.Range(i, trimmed.indexOf('['), i, trimmed.indexOf(']') + 1), `Keyword ID should include category: [category.keywordId]`, vscode.DiagnosticSeverity.Warning));
                }
            }
        }
        ...
        if (currentSection === 'ACTIONS' && braceDepth === 0) {
            ...
            if (actionDefMatch) {
                ...
                if (!actionId.includes('.') && !['shake', 'pulse', 'glow', 'fade', 'highlight', 'wiggle'].includes(actionId)) {
                    problems.push(new vscode.Diagnostic(new vscode.Range(i, 1, i, 1 + actionId.length), `Action ID should follow naming convention: @category.actionId`, vscode.DiagnosticSeverity.Information));
                }
            }
        }
        ...
        if (trimmed.startsWith('importance:')) {
            ...
            if (value && !validImportance.includes(value)) {
                problems.push(new vscode.Diagnostic(new vscode.Range(i, trimmed.indexOf(':') + 1, i, trimmed.length), `Invalid importance value. Valid values: ${validImportance.join(', ')}`, vscode.DiagnosticSeverity.Warning));
            }
        }
    }
    // Report unclosed braces
    if (braceDepth > 0) {
        problems.push(new vscode.Diagnostic(new vscode.Range(currentBlockStart, 0, currentBlockStart, 1), `Unclosed brace - ${braceDepth} brace(s) not closed`, vscode.DiagnosticSeverity.Error));
    }
    // Warn if no source defined
    if (!hasSource) {
        problems.push(new vscode.Diagnostic(new vscode.Range(0, 0, 0, 1), 'Missing "source" property in META section', vscode.DiagnosticSeverity.Warning));
    }
}
/**
 * Generate HTML for keyword info panel
 */
function getKeywordInfoHtml(keyword) {
    ...
    provideCompletionItems(document, position, token, context) {
        ...
        if (keywordSourceMatch) {
            ...
            sources.forEach(source => {
                ...
                item.documentation = new vscode.MarkdownString(sourceFile?.patientInfo
        ...
        if (keywordIdMatch) {
            ...
            keywords.forEach(kw => {
                ...
                const item = new vscode.CompletionItem(importanceIcon ? `${importanceIcon} ${keywordId}` : keywordId, vscode.CompletionItemKind.Value);
                item.detail = `${kw.importance.toUpperCase()} | ${kw.displayText}`;
                item.documentation = new vscode.MarkdownString(`**${kw.displayText}**\n\n${kw.description}\n\n` +
                    `ID: \`${kw.id}\`\n\n` +
                    `Importance: \`${kw.importance}\`` +
                ...
                if (position.character >= startIndex && position.character <= endIndex) {
                    ...
                    if (parsed) {
                        ...
                        if (keyword) {
                            ...
                            if (keyword.effects) {
                                if (keyword.effects.focusCost)
                                    effectsList.push(`Focus: ${keyword.effects.focusCost}`);
                                if (keyword.effects.rapportChange)
                                    effectsList.push(`Rapport: ${keyword.effects.rapportChange > 0 ? '+' : ''}${keyword.effects.rapportChange}`);
                                if (keyword.effects.reveals?.length)
                                    effectsList.push(`Reveals: ${keyword.effects.reveals.length}`);
                                if (keyword.effects.contradicts?.length)
                                    effectsList.push(`Contradicts: ${keyword.effects.contradicts.length}`);
                            }
