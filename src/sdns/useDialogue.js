/**
 * useDialogue - React hook for the SDNS dialogue system
 * Session Dialogue and Narration System - React Integration
 * 
 * Provides easy integration with React components
 * 
 * @module sdns/useDialogue
 * @version 1.0.0
 */
...
export function useDialogue(gameState, actions) {
    ...
    useEffect(() => {
        ...
        engineRef.current.onRapportChange = (amount) => {
            console.log(`[SDNS] Rapport changed by ${amount}`);
        };
        ...
        engineRef.current.onBlockEnd = (blockName) => {
            console.log(`[SDNS] Block "${blockName}" ended`);
        };
    ...
    const loadDialogueFromUrl = useCallback(async (url) => {
        try {
            ...
            console.error('[SDNS] Failed to load dialogue:', error);
        }
    }, [loadDialogue]);
