...
exports.SynsCompletionProvider = void 0;
const vscode = __importStar(require("vscode"));
const constants_1 = require("../utils/constants");
class SynsCompletionProvider {
    ...
    getDirectiveCompletions() {
        return constants_1.DIRECTIVES.map(directive => {
            const item = new vscode.CompletionItem(`@${directive}`, vscode.CompletionItemKind.Keyword);
            const doc = constants_1.DOCUMENTATION[`@${directive}`];
            if (doc) {
    ...
    getMoodCompletions() {
        return constants_1.MOODS.map(mood => {
            const item = new vscode.CompletionItem(mood, vscode.CompletionItemKind.EnumMember);
            item.detail = 'Mood';
            item.documentation = constants_1.MOOD_DESCRIPTIONS[mood] || mood;
            item.insertText = mood + ')';
            return item;
        });
    }
    getSpeakerCompletions() {
        return constants_1.SPEAKERS.map(speaker => {
            const item = new vscode.CompletionItem(speaker, vscode.CompletionItemKind.Class);
            const doc = constants_1.DOCUMENTATION[speaker];
            if (doc) {
                item.detail = doc.signature;
                item.documentation = new vscode.MarkdownString(doc.description);
            }
    ...
    getConditionCompletions(document) {
        const items = [];
        // Built-in variables
        for (const variable of constants_1.BUILTIN_VARIABLES) {
            const item = new vscode.CompletionItem(variable, vscode.CompletionItemKind.Variable);
            const doc = constants_1.DOCUMENTATION[variable];
            if (doc) {
                item.detail = doc.signature;
        ...
        // Built-in functions
        for (const func of constants_1.BUILTIN_FUNCTIONS) {
            const item = new vscode.CompletionItem(func, vscode.CompletionItemKind.Function);
            const doc = constants_1.DOCUMENTATION[func];
            if (doc) {
                item.detail = doc.signature;
                item.documentation = doc.description;
            }
    ...
    createOperatorSnippet(label, snippet, detail) {
        const item = new vscode.CompletionItem(label, vscode.CompletionItemKind.Snippet);
        item.insertText = new vscode.SnippetString(snippet);
        item.detail = detail;
        return item;
    }
    getSymptomCompletions(document) {
    ...
    getKeywordCompletions(document) {
    ...
    getFunctionArgCompletions(document, linePrefix) {
...
exports.SynsCompletionProvider = SynsCompletionProvider;
